...
var closebtn= Ext.get('close-btn');
	...
	var icad=Ext.getCmp('insuredaccountid').getValue();
                ...
                items: [
				  {
                    ...
                    handler: function(grid, rowIndex, colIndex) {
						...
						var transactionTable=rec.get('transtbl');
				        deleteRecordOnconfirmation(transactionTable,ridtr,paymentgridstore);
...
if(transactionTable=='accounts_cashtrans'){
accounts_cashtransForm('detailinfo','updateload',ccrecordid);
}
if(transactionTable=='accounts_banktrans'){
accounts_banktransForm('detailinfo','updateload',ccrecordid);
}
if(transactionTable=='accounts_checkdeposit'){
accounts_checkdepositForm('detailinfo','updateload',ccrecordid);
}
...

{
	...
	},
	{
		...
		buttons: [{
			...
			id: 'savetpaymentbtn',
			handler:function(){
			            var	accountscategory_id=57;
						var	accaccount_id=Ext.getCmp('insuredaccountid').getValue();	
					...
					var account= Ext.getCmp('insuredaccountid').getValue();
					var	currency_id	=Ext.getCmp('tcurrency_id').getValue();	
					saveExpense(amount,currency_id,transaction_date,naration,account,58);
			          } 
