<?php
$dated=date('Y-m-d');
echo "function showChangePWDComments(title,table,
primary_key,
primary_keyfield,
actionField,
actionFieldValue){
...
Ext.tip.QuickTipManager.init();
	...
	
	{
		...
		var confirm=Ext.getCmp('confirmpassword').getValue();
		
		if((oldpassword.length>0)&&(newpassword.length>0)&&(confirm.length>0)){
		       if(confirm==newpassword){
				changePasswordByUser(oldpassword,newpassword);
				}else{
				showparserror('New and confirm password do not match!','Confirm Password Error!')
				}
				
				}
		else{
		showparserror('Password fields cannot be empty!','Incomplete Details!')
		}
        }},
		{
		xtype: 'button',
...
showChangePWDComments('Change Password','medicallab_resultreview',2,'queue_id','is_approved','true')";

//title,table,primary_key,primary_keyfield,date_action,actionField,actionFieldValue
?>