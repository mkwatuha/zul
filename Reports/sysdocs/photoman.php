...
$datafolder=trim($_GET['ftbl']);
$datafolder=str_replace('_','',$datafolder);
$datafolder=strtolower($datafolder);
$tblcaption='';
$dir = $datafolder.'/';//"images/";
$images = array();
$d = dir($dir);
while($name = $d->read()){
    if(!preg_match('/\.(jpg|gif|png|doc)$/', $name)) continue;
    $size = filesize($dir.$name);
    $lastmod = filemtime($dir.$name)*1000;
	
	if(preg_match('/\.(doc)$/', $name)){
	$pictureurl='../sysdocs/images/kids_hug.jpg';
	}else{
	$pictureurl='../sysdocs/'.$dir.$name;
	}
    $images[] = array('name'=>$name, 'size'=>$size,
	
			'lastmod'=>$lastmod, 'url'=>$pictureurl);
}
$d->close();
