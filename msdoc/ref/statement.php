...
$document = $PHPWord->loadTemplate('Statementplv2.docx');
...
$landlordinfo=fillPrimaryData('housing_housinglandlord',$primaryselector);
//=getLandlordPrepayments($landlordinfo['housinglandlord_name'],51);

             $foundrecordsarr['vouchernumber']
			 .'|'.$foundrecordsarr['amount'].'|'.
			 $foundrecordsarr['naration'].'|'.
			  $foundrecordsarr['date_created'];
			 $i++;
			 
$trnas=getLandlordBankCashTransactions($landlordinfo['housinglandlord_name'],51);
$detailsnaration='';
$transactionDate='';
$transactionMonth='';
$transamount='';
...
$totalamnt=0;
for($it=0;$it<15;$it++){
foreach($trnas as $transactiondetails){
$tn=explode('|',$transactiondetails);
$totalamnt=$totalamnt+$tn[1];
$transvouchercheck.=$tn[0]."<w:br/>";
$transamount.=number_format($tn[1],2)."<w:br/>";
$detailsnaration.=$tn[2]."<w:br/>";
$transactionMonth.=trim($tn[3])."<w:br/>";
$transactionDate.=$tn[3]."<w:br/>";
}
}
...
$document->setValue('Value81', $personname);
$dbdate=date('d-m-Y');
...
$document->setValue('Value8', $transactionMonth);
$document->setValue('Value9', $transactionDate);
$document->setValue('Value84', $transvouchercheck);
   
$document->setValue('Value10', $transamount);
$document->setValue('Value11', $detailsnaration);
$document->setValue('Value12', $political);
$document->setValue('Value13', number_format($totalamnt,2));
...
$document->save('statement.doc');
echo "function OpenDebitNoteR(){
window.location.assign('../msdoc/ref/statement.doc');
}OpenDebitNoteR();";
?>